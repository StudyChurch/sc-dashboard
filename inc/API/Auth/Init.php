...
namespace StudyChurch\API\Auth;
...
class Init {
	...
	public static function rest_api_auth_handler( $input_user ){
		...
		$api_request = ( defined( 'REST_REQUEST' ) && REST_REQUEST ) && ( ( false !== strpos( $route, studychurch()->get_api_namespace() ) || ( false !== strpos( $route, 'buddypress' )) ) );
		...
		if ( ! $user ) {
			self::$_errors[] = new WP_Error( 'user_not_found', __( 'The username or email you entered does not match an existing user.' ) );
			return $input_user;
		}
		...
		self::$_errors[] = new WP_Error( 'incorrect_password', __( 'The password you entered was incorrect.' ) );
	...
	public static function get_password_schema() {
		...
		return apply_filters( 'studychurch_get_password_schema', $schema );
	}
